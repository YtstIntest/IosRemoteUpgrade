...
open class RemoteUpdateManage: NSObject {
    ...
    public func configEnvironment(configs:Config) -> Void {
        config=configs
    }
    
    
    public func getUpdateList(vin:String,uDate:String,success:@escaping (CurrentVehicleTaskModel) -> (Void),failed:@escaping (String) -> (Void)) -> Void {
    ...
    ///   - uDate: 日期时间
    ///   - taskcarId: 车辆升级任务id
    public func queryCarUpdateTask(vin:String,uDate:String,success:@escaping (CurrentVehicleTaskModel) -> (Void),failed:@escaping (String) -> (Void)) -> Void {
